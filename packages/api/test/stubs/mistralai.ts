// packages/api/test/stubs/mistralai.ts
/**
 ...
 * that transitively imports `@librechat/agents` at all (Google config, agent
 * initialization, memory, etc.) died at collection with
 ...
 * So Jest Can Load Them"). That fix lives in the agents repo's dev tooling
 * and is not shipped in the published npm package, so consumers (this fork)
 * need the equivalent stub in their own jest config.
 *
...
export class ChatMistralAI {
  constructor() {
    throw new Error(
      'ChatMistralAI is stubbed under Jest (packages/api/test/stubs/mistralai.ts) because ' +
